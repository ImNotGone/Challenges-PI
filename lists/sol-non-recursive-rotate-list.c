...
static unsigned int get_length(TList list) {
    if(list == NULL) return 0;
    return get_length(list->tail) + 1;
}

TList rotate_list(TList list, unsigned int times) {
    unsigned int length = get_length(list);

    unsigned int cut_pos = times % length;

    TList last_node = list;
    ...
    for (int i = 0; i < cut_pos; i++) {
        cut_node = cut_node->tail;
    }

    TList new_head = cut_node->tail;
    cut_node->tail = NULL;
