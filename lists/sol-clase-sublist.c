...
TList sublist(TList list, int start, int end) {
    ...
    // si todavia no estoy parado en el elemento actual
    // decremento ambos
    if(start > 0) {
        return sublist(list->tail, start - 1, end - 1);
    }
    // start = 0 porque sino hubiera entrado en el if de arriba
    // o en el if del llamado recursivo
    // end >= 0 porque sino hubiera entrado en el if de arriba

    TList aux = malloc(sizeof(TNode));
    aux->elem = list->elem;
    aux->tail = sublist(list->tail, start, end - 1);
    return aux;
}
